...
def main():
    ...
    node.declare_parameter("position1", [0.35, 0.1, 0.68])
    node.declare_parameter("position2", [0.194, -0.43, 0.701])
    node.declare_parameter("positiond", [-0.37, 0.12, 0.397] )
    ...
    positiond = node.get_parameter("positiond").get_parameter_value().double_array_value
    ...
    # Move to pose
    node.get_logger().info(
        f"Moving to {{position: {list(positiond)}, quat_xyzw: {list(quat_xyzw)}}}"
    )
    moveit2.move_to_pose(position=positiond, quat_xyzw=quat_xyzw, cartesian=cartesian)
    moveit2.wait_until_executed()
