...
def detect_aruco(image):
    ...
    if ids is not None and len(ids) > 0:
        ...
        for i, marker_id in enumerate(ids):
            ...
            if area[0] > aruco_area_threshold:
                ...
                image_to_show=aruco_display(corners, ids, rejected, gray_image)     
                ...
                # Draw frame axes
                rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(corners[i], 0.02, cam_mat, dist_mat)
                cv2.drawFrameAxes(image_to_show, cam_mat, dist_mat, rvec, tvec,0.1)
    ...
    return center_aruco_list, distance_from_rgb_list, angle_aruco_list, width_aruco_list, ids,image_to_show
...
class aruco_tf(Node):
    ...
    def process_image(self):
        ...
        print(self.cv_image)
        center_aruco_list, distance_from_rgb_list, angle_aruco_list, width_aruco_list, ids,image_to_show = detect_aruco(self.cv_image)
        #$print("ids:", ids)
        ...
        if ids is not None: 
            ...
            for marker_id in ids:
                ...
                angle=angle_aruco_list[marker_id]
                ...
                distance_from_rgb = self.depth_image[int(center_aruco_list[marker_id][1])][int(center_aruco_list[marker_id][0])] / 1000
                ...
                x = distance_from_rgb * (sizeCamX - center_aruco_list[marker_id][0] - centerCamX) / focalX
                y = distance_from_rgb * (sizeCamY - center_aruco_list[marker_id][1] - centerCamY) / focalY
                z = distance_from_rgb

                #   ->  Now, mark the center points on image frame using cX and cY variables with help of 'cv2.cirle' function 
                cv2.circle(image_to_show, center_aruco_list[marker_id], 5, (0, 0, 255), -1)
            ...
            if self.cv_image is None:
                 ...
                 cv2.imshow('Detected Markers', image_to_show) 
                 cv2.waitKey(100)
