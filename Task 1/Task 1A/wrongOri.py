    ...
    def process_image(self):
        ...
        if ids is not None: 
            ...
            while i < len(angle_aruco_list):
                ...
                quaternion /= np.linalg.norm(quaternion) #new
                print("i val:", i)
                print("Corrected angle: ",corrected_angle)
                #new 
                desired_rotation_angle_roll = np.deg2rad(90)  # Adjust this angle as needed
                desired_rotation_angle_pitch = np.deg2rad(0)
                desired_rotation_angle_yaw = np.deg2rad(90)
                desired_rotation_quaternion = tf_transformations.quaternion_from_euler(desired_rotation_angle_roll, desired_rotation_angle_pitch, desired_rotation_angle_yaw )
                rotated_quaternion = tf_transformations.quaternion_multiply(desired_rotation_quaternion, quaternion)
                ...
                tf_msg.child_frame_id = 'cam_' + str(ids[i][0])
                tf_msg.transform.translation.x = x
                tf_msg.transform.translation.y = y
                tf_msg.transform.translation.z = z 
                tf_msg.transform.rotation.x = float(np.sin(corrected_angle/2)/2**0.5)#rotated_quaternion[0]
                tf_msg.transform.rotation.y = float(np.sin(corrected_angle/2)/2**0.5)#rotated_quaternion[1]
                tf_msg.transform.rotation.z = float(np.cos(corrected_angle/2)/2**0.5)#rotated_quaternion[2]
                tf_msg.transform.rotation.w = float(np.cos(corrected_angle/2)/2**0.5)#rotated_quaternion[3]
                ...
                try:
                    ...
                    base_to_obj = self.tf_buffer.lookup_transform('base_link','cam_'+ str(ids[i][0]), Time())
